...
The YouTube video [The cure for AI slop is a 1986 aircraft manual](https://www.youtube.com/watch?v=uJblcC4lKYw) introduced me to the concept of STE. I have a product coming out soon, and after watching it I wanted my instructional documents to adhere to the STE standard. Not because of regulatory reasons - I just thought it was a neat idea and effective for my customers.

The standard is only useful if the writing actually follows it. The point of this skill is to be a dependable way to implement the standard: it does not trust the model to remember 53 rules and a controlled dictionary from training data. It loads the rule index, checks every word against the dictionary, and runs a mechanical linter on the result before anything is called done.

## What It Does

This skill rewrites Non-STE text into compliant Simplified Technical English:
...
The skill does not expect the agent to hold all 53 rules in context. `SKILL.md` is a compact router: a rule index, the mandatory verification protocol, and pointers to per-section deep dives. The agent reads a deep dive only when a violation in that section is found. Word checks run through `lookup.py` one word at a time instead of reading the 86 KB dictionary, and the `ste_check.py` linter handles the machine-checkable rules in one pass.

### The write-then-verify loop

The skill enforces a fixed protocol - write, lint, fix one rule at a time, re-lint to zero, then judge the rest:
...
The examples below are instructional text. That is what ASD-STE100 was designed for: maintenance and operating procedures that a reader must follow exactly, with no room for misunderstanding. Each Before text is a real Non-STE paragraph; each After text is the STE rewrite that this skill produces and verifies.
...
## Comparison with Other STE Skills
