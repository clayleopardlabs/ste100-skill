...
**Technical details (Rule 4.2):** STE simplifies phrasing, never information. Never drop, generalize, or round a technical detail to make text shorter. Technical details are: numbers with units (12-15 mmHg, 10 mm, 40 °C), ranges (3-0), acronyms (CO2, DC), brand and material names (Vicryl, Monocryl), capitalized names (McBurney's point, Kelly clamp, Veress needle), hyphenated compound terms (0-Vicryl, low-speed), and conditions ("if the port site is more than 10 mm"). Mechanical check: `python ste_check.py --details <original> <corrected>` — extracts every such detail from the original and flags any that are missing from the corrected text. A range may be spelled out ("12-15 mmHg" -> "12 to 15 mmHg"); every other detail must survive verbatim. 0 errors, 0 warnings is required.
