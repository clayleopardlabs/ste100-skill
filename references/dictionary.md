# STE100 Controlled Dictionary (Part 2)
...
* **BY (prep)** — Mathematical function word; Use as a function word to show agency when a passive construction is necessary in descriptive writing.
* **CALIBRATION (n)** — The procedure that calibrates
* **CANNOT (v)** — Helping verb that means to not be able to
...
* **CONDITION (n)** — 1. Something that is necessary for what occurs; 2. The state of an item; Use IF (conj) for “on the condition that.”
...
* **DATA (n)** — 1. Known facts; 2. Numbers and symbols used by computers; You can also use the singular.
...
* **FACE (n)** — The front surface of an object; This word is a technical name when it is referred to a part of the body.
...
* **FREE (adj)** — That can move easily; Do not use compound adjectives with free, for example, lintfree, dustfree, unless they are technical names.
...
* **FUNCTION (n)** — The purpose of an operation or an item
...
* **LENGTH (n)** — 1. The distance from one end to the other end; 2. The longer or longest of the three dimensions of an object
...
* **MANUAL (n)** — A publication that supplies data related to a special subject
...
* **MATERIAL (n)** — 1. A substance or something from which a thing is made; 2. Something made to do a task
...
* **MORE (adj)** — Refer to MANY (adj) and MUCH (adj)
* **MORE (adv)** — By a larger dimension, value, quantity, number, or degree
* **MOST (adj)** — Refer to MANY (adj) and MUCH (adj)
* **MOVABLE (adj)** — That can move if necessary
* **MOVEMENT (n)** — A change of position or location
* **MUCH (adv)** — To a great degree or extent
* **MUST (v)** — Helping verb that shows obligation
...
* **ONE (pron)** — That refers to a single person or object You can also use this word as a technical name.
...
* **OUT OF (prep)** — 1. Through or from the inside to the outside; 2. Not in
...
* **OVER (prep)** — To indicate a position higher than or above land or water Use this word only for movement above land or water. Not permitted for other meanings. Refer to the word “over (prep)” below.
...
* **PART (n)** — 1. A constituent of a machine or other equipment; 2. A piece or section of a whole
...
* **POINT (n)** — 1. A sharp or tapered end; 2. An accurate location
...
* **THAT (conj)** — Function word that starts a subordinate clause
...
* Subject → **Change**
* Rule 2.2 → **When a technical name has more than three words, write it in full.** **Then you can simplify it as follows:** **- Give a shorter name** **or** **- Use hyphens (-) between words that are used as a single unit.**
* Active voi → **ce**
* How to de → **scribe an action**
* Rule 8.3 → **You can use parentheses:** **- To make references to illustrations or text** **- To include letters or numbers that identify items on an illustration or** **in a text** **- To identify the steps in a procedure** **- To include abbreviations** **- To give the singular and plural forms of a noun at the same time** **- To explain words or a part of a sentence** **- To include an alternative.**
* Rule 8.6 → **Count each of these as one word:** **- Numbers** **- Units of measurement** **- Abbreviations** **- Alphanumeric identifiers** **- Quoted text** **- Titles, headings, placards, and labels.**
* Consisten → **t style**
...
* uncovered (v) → COVER (TN)
* manufacture (v) → MAKE (v)
* previous (adj) → BEFORE (conj)
* abaft (prep) → AFT OF (prep)
* abandon (v) → STOP (v); GO (v)
* abate (v) → DECREASE (v)
* ability (n) → CAN (v)
* able (adj) → CAN (v)
...
* accommodate (v) → LET (v)
* accomplish (v) → DO (v); You can use a more specific command verb.; COMPLETE (v)
...
* act (v) → Use a specific action verb.
* action (n) → STEP (n); PROCEDURE (n)
* activate (v) → START (v); OPERATE (v); CONNECT (v)
* active (adj) → ON (adj); OPERATE (v)
* activity (n) → WORK (n)
...
* adverse (adj) → BAD (adj) Give accurate and correct conditions if possible.
* advisable (adj) → RECOMMEND (v)
* advise (v) → TELL (v); RECOMMEND (v)
* affect (v) → EFFECT (n) Be specific if possible.
...
* already (adv) → IN PROGRESS (adv); NO OTHER; Frequently, an alternative for this word is not necessary.
* alter (v) → CHANGE (v)
...
* annotation (n) → WRITE (v)
* annunciate (v) → SHOW (v)
* another (adj) → ONE (TN) MORE; DIFFERENT (adj); ADD (v); AGAIN (adv)
* any (adj) → Frequently, an alternative for this word is not necessary.
* any (pron) → ONE (TN)
...
* arrive (v) → GET (v)
* article (n) → OBJECT (n)
* as (conj) → BECAUSE (conj); WHILE (conj) You can use:; AS FOLLOWS; AS NECESSARY; AS SHOWN
...
* assist (v) → HELP (v)
* assistance (n) → AID (n)
* associated (adj) → CORRECT (adj); RELATED (adj); ITS, THEIR (pron)
* assume (v) → THINK (v)
* assure (v) → MAKE SURE (v)
* as to (prep) → ABOUT (prep)
* asymmetric (adj) → NOT SYMMETRICAL
* at least (adv) → MINIMUM (adj); MINIMUM (n)
...
* break (n) → STOP (v)
* breakdown (n) → STOP (v)
* brightness (n) → INTENSITY (n); BRIGHT (adj)
...
* carry out (v) → DO (v); You can use a more specific command verb.; MEASURE (v)
* case (n) → CONDITION (n)
...
* cease (v) → STOP (v)
* center (v) → CENTER (TN)
* centralize (v) → CENTER (TN); NEUTRAL (TN)
...
* clamp (v) → ATTACH (v) (WITH A CLAMP [TN] OR CLAMPS [TN]); PUT (v)
* classification (n) → CLASS (TN); CATEGORY (TN)
* cleanliness (n) → CLEAN (adj)
* clear (v) → CLEAN (v); You can also use a different construction.
* click (v) → CLICK (n); You can use this word as a technical verb for computer processes and applications.
...
* coil (v) → WIND (v)
* coincide (v) → ALIGN (v)
* coincident (adj) → SYNCHRONIZED (adj)
...
* commence (v) → START (v)
* common (adj) → SAME (adv)
* communicate (v) → TELL (v); SPEAK (v)
* communication (n) → SPEAK (v); HEAR (v)
...
* comply (v) → OBEY (v); Used in regulatory language. Refer to Rule 1.12.
...
* consult (v) → REFER (v)
* contact (v) → TOUCH (v); SPEAK (v); WRITE (v)
...
* danger (n) → RISK (n); DANGEROUS (adj) This word is a technical name when it identifies a safety instruction
* dangerously (adv) → DANGEROUS (adj)
* deactivate (v) → STOP (v); DISCONNECT (v); ISOLATE (v)
* deadly (adj) → KILL (v)
...
* delay (v) → AFTER (prep)
* delete (v) → ERASE (v); REMOVE (v); You can use this word as a technical verb for computer processes and applications.
* delicate (adj) → EASILY DAMAGED
* deliver (v) → SUPPLY (v)
* delivery (n) → SUPPLY (v)
* demand (v) → MANDATORY (adj)
* denote (v) → SHOW (v)
* dent (v) → DENT (TN)
* depend (v) → IF (conj)
...
* drag (v) → PULL (v); You can use this word as a technical verb for computer processes and applications.
...
* either (conj) → ONE (TN) OF THE TWO (TN)
* either (pron) → ONE (TN) OF THE TWO (TN)
* ejection (n) → EJECT (v)
...
* entail (v) → MUST (v)
* enter (v) → GO INTO; RECORD (v); ENTRY (n); You can use this word as a technical verb for computer processes and applications.
...
* essential (adj) → NECESSARY (adj); MUST (v)
...
* exceed (v) → MORE THAN
* except (prep) → To replace this word, use a different construction. Be specific.
* exception (n) → To replace this word, use a different construction. Be specific.
...
* facility (n) → Be specific. Use the name of the facility as a technical name.
* factor (n) → CAUSE (n)
* fail (v) → IF … NOT; FAILURE (TN); UNSATISFACTORY (adj)
* failed (adj) → DEFECTIVE (adj); UNSERVICEABLE (adj)
* failure (n) → IF … NOT; This word when it means a performance error or loss of serviceability is a technical name.
...
* few (adj) → SMALL NUMBER If possible, give a specific number.
* few (a few) (adj) → SOME (adj); If this alternative is not sufficient, give the range, number, or quantity.
...
* flatness (n) → FLAT (adj)
* flatten (v) → FLAT (adj)
* flaw (n) → DAMAGE (n); DEFECTIVE (adj) Be specific if possible.
* flex (v) → BEND (v)
...
* garble (adj) → CLEAR (adj); CLEARLY (adv)
* garner (v) → COLLECT (v)
* gaseous (adj) → GAS (n)
* gash (v) → DAMAGED (adj)
* gather (v) → COLLECT (v)
...
* generous (adj) → LARGE (adj); THICK (adj); Be specific if possible.
* generously (adv) → LARGE (adj); THICK (adj) Be specific if possible.
* gentle (adj) → CAREFULLY (adv); LIGHTLY (adv)
* gently (adv) → CAREFULLY (adv); LIGHTLY (adv)
* genuine (adj) → CORRECT (adj)
* get (v) → BECOME (v)
* get away (v) → GO AWAY (v)
* get down (v) → DECREASE (v); GO DOWN (v)
* get into (v) → GO INTO (v)
* get off (v) → MOVE OFF (v)
...
* halve (v) → HALF (TN)
* hamper (v) → PREVENT (v)
* handle (v) → MOVE (v); TOUCH (v); CAREFUL (adj) This word can be part of safety sign (rule 8.6).
...
* imperative (adj) → NECESSARY (adj); MANDATORY (adj); MUST (v)
* implement (v) → DO (v)
...
* inactive (adj) → OFF (adj); NO EFFECT; NOT IN OPERATION; NOT OPERATE
...
* interference (n) → HIT (v); CATCH (v); TOUCH (v); This word can be a technical name or part of a technical name.
...
* later (adv) → SUBSEQUENTLY (adv); WHEN (conj) Give the condition or time, if possible.; AFTER (prep)
* lay (v) → PUT (v)
* lead (v) → PUT (v)
* leak (v) → LEAK (n)
...
* operational (adj) → SERVICEABLE (adj); OPERATE (v)
* opportunity (n) → Be specific if possible.
* option (n) → POSSIBLE (adj)
* order (n) → SEQUENCE (n)
...
* over (prep) → ABOVE (prep); ALONG (prep); ON (prep); MORE THAN
* overfill (v) → TOO MUCH
...
* people (n) → PERSONS (n); PERSONNEL (n)
* per (prep) → FOR EACH; REFER (v)
* perforated (adj) → HOLE (n)
* perform (v) → DO (v); You can use a more specific command verb.
...
* probable (adj) → POSSIBLE (adj); RISK (n)
* probe (v) → FIND (v) (WITH A PROBE [TN])
* proceed (v) → CONTINUE (v)
* process (n) → PROCEDURE (n); This word is a technical name when it means a sequence of changes that occur and cause a result.
* process (v) → PUT (v)
...
* reactivate (v) → OPERATE (v); START (v)
...
* relay (v) → SEND (v)
* relevant (adj) → RELATED (adj); THEIR (pron); ITS (pron)
* relieve (v) → RELEASE (v)
* remain (v) → STAY (v)
...
* reuse (v) → USE (v)
* reveal (v) → SHOW (v)
* reverse (adj) → OPPOSITE (adj); This word is part of a technical name when it refers to rearward thrust.
* reverse (v) → OPPOSITE (adj)
* review (n) → INSPECTION (n)
* rewind (v) → WIND (v)
* rework (v) → Refer to technical verbs, manufacturing process.
* rig (v) → ADJUST (v)
...
* sever (v) → CUT (v)
* several (adj) → SOME (adj); If this alternative is not sufficient, give the range, quantity, number, or minimum value.
...
* solve (v) → SOLUTION (n)
* soon (adv) → TIME (n); Give the time if possible.
* sound (v) → OPERATE (v)
* space (v) → DISTANCE (n)
...
* spark (v) → SPARK (n)
* specific (adj) → SPECIAL (adj); SPECIFIED (adj)
...
* switch (v) → SET (v)
* switch off (v) → STOP (v); SWITCH (TN)
* switch on (v) → START (v); SWITCH (TN)
...
* undertake (v) → DO (v); Use a specific action verb.
* undo (v) → LOOSEN (v)
...
* unscrew (v) → REMOVE (v); LOOSEN (v); TURN (v)
...
* utilization (n) → USE (v)
* utilize (v) → USE (v)
* valid (adj) → CORRECT (adj); APPLICABLE (adj); This word becomes part of a technical name when associated to an official document.
...
* vital (adj) → IMPORTANT (adj); MANDATORY (adj); NECESSARY (adj)
...
* yet (conj) → BUT (conj); AT THIS TIME
