...
**Target: 7 violations** (4 ERROR, 3 WARNING). The paragraph is otherwise perfect STE.

| # | Line | Offending text | Rule | Correction |
|---|------|----------------|------|------------|
| 1 | 10 | "Swap the cover into position." — word **Swap** | Rule 1.1/1.6 (unapproved word) | Replace with an approved word: "Interchange the cover into position." or "Put the cover into position." |
...
| 5 | 4 | "Hold the wrench with the left hand while **holding** the valve." — **-ing form "holding"** | Rule 3.5 (-ing form) | Rewrite with an approved form: "Hold the wrench with the left hand. Hold the valve." |
...
**Pass criteria:** The model identifies all seven violations and corrects each. Final text: 0 errors, 0 warnings from ste_check.py. Any other change that introduces a new violation fails the test.
