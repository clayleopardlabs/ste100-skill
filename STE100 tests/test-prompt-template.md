...
- If two consecutive lint outputs are IDENTICAL (same violations, same lines), you are stuck: change your FIX (re-read the section file, pick a different correction), do not re-lint the same text.
- After 6 linter runs, STOP regardless: deliver your best draft, listing any remaining violations and why you could not resolve them.
