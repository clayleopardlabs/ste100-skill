...
1. **In the skill** (SKILL.md Phase 4): the fix loop is bounded — at most 6 linter runs; if two consecutive lint outputs are identical the agent is stuck and must change its FIX, not re-lint; after 6 runs it must stop and deliver the best draft with remaining violations reported, never loop.
...
A run is declared STUCK (and killed) if the agent exceeds the cap or produces identical consecutive lint outputs. A bounded imperfect answer always beats an infinite loop.
